...
This fork ships permission modes, isolated subagents, and bounded `/goal` continuation by default while retaining Pi's extension-first design. Other workflows such as plan mode remain available through custom or third-party extensions.
...
- **Escape** aborts the current turn and immediately continues with queued messages
- **Alt+Up** retrieves queued messages back to editor

On Windows Terminal, `Alt+Enter` is fullscreen by default. Remap it in [docs/terminal-setup.md](docs/terminal-setup.md) so pi can receive the follow-up shortcut.

Configure delivery in [settings](docs/settings.md): `steeringMode` and `followUpMode` can be `"all"` (default, delivers all queued at once) or `"one-at-a-time"` (waits for a response between messages). `transport` selects provider transport preference (`"sse"`, `"websocket"`, or `"auto"`) for providers that support multiple transports.
...
**Sub-agents stay extension-based.** This fork enables the bundled subagent extension by default, with isolated contexts plus inherited model, thinking level, and permission mode. Disable it with `--exclude-tools subagent` or disable extension discovery with `--no-extensions`.
...
| `--permission-mode <mode>` | `manual`, `read-only`, `auto-read-only`, `auto`, or `skip` |
